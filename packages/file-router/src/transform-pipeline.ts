...
export class TransformPipeline {
  ...
  private generateJobId(fileKey: string, operations: TransformOperation[], params: TransformParams): string {
    const paramsHash = Buffer.from(JSON.stringify(params)).toString('base64').replace(/[+/=]/g, '');
  ...
  async handleTransform(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!fileKey || !projectId) {
        return res.status(400).json({ error: 'File key and project ID required' });
      }
...
export function createTransformPipeline(config: TransformPipelineConfig): TransformPipeline {
  return new TransformPipeline(config);
}

// Factory function to create transform middleware
export function createTransformMiddleware(pipeline: TransformPipeline): TransformMiddleware {
