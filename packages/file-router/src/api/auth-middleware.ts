...
export class AuthMiddleware {
  ...
  rateLimit = (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.setHeader(
        ...
        Math.max(0, maxRequests - rateLimitInfo.requests)
      );
  ...
  requireProjectAccess = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      !userProjectId ||
      !requestProjectId ||
      userProjectId !== requestProjectId
    ) {
  ...
  cors = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.header(
      ...
      "Content-Type, Authorization, X-API-Key, X-User-Id, X-Project-Id, X-Organization-Id, X-User-Tier"
    );
    res.header(
